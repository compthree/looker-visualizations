  ...
  for (let row of stateData) {
    for (let colName of Object.keys(row)) {
      if (isUrlCheck(row[colName].value)) {
        ...
        // console.log(`fcol url ret: url=${tmpImageColData.type.url}, b64=${tmpImageColData.type.base64}, ${tmpImageColData.name}`);
        return tmpImageColData;
      } else if (isBase64StringCheck(row[colName].value)) {
        tmpImageColData.type.base64 = true;
        tmpImageColData.name = colName;
        // console.log(`fcol set b64 - no ret: url=${tmpImageColData.type.url}, b64=${tmpImageColData.type.base64}, ${tmpImageColData.name}`);
      }
    }
    // stop looping rows if we found a valid image column
    if (tmpImageColData.name) {
      // console.log(`fcol after col for loop: url=${tmpImageColData.type.url}, b64=${tmpImageColData.type.base64}, ${tmpImageColData.name}`);
      return tmpImageColData;
    }
  }
  // resets this.state.imageColData to falsy if no valid image col was found
  // console.log(`fcol final end ret: url=${tmpImageColData.type.url}, b64=${tmpImageColData.type.base64}, ${tmpImageColData.name}`);
  return tmpImageColData;
}
...
export default class ImageViewer extends React.Component {
  ...
  render() {
    ...
    if (!this.state.imageColData.name) {
      // TODO: fix setState, move out to lifcycle
      // this.setState({imageColData: findImageCol(this.state.data)});
      this.state.imageColData = findImageCol(this.state.data);
      // console.log(`ln101 first: url=${this.state.imageColData.type.url}, b64=${this.state.imageColData.type.base64}, ${this.state.imageColData.name}`);
    }
    ...
    if (typeof this.state.data[0][this.state.imageColData.name] === 'undefined') {
      ...
      // this.setState({imageColData: findImageCol(this.state.data)});
      this.state.imageColData = findImageCol(this.state.data);
      // console.log(`ln118 first: url=${this.state.imageColData.type.url}, b64=${this.state.imageColData.type.base64}, ${this.state.imageColData.name}`);
    }
    ...
    // tableRows = SAMPLE_IMAGES.map((catUrl, idx) => {
    //   return (
    //     <div key={idx}>
    //       <img src={catUrl} />
    //     </div>
